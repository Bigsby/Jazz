function RenderStave(elementId) {
    var vf = new Vex.Flow.Factory({ renderer: { selector: elementId } });
...
function RenderEasy(options) {
    var vf = new Vex.Flow.Factory(options);
    ...
    var stave = system.addStave({
        voices: [
            voice([
                //notes('D5/q[id="m1a"]'),
                [
                    new VF.StaveNote({ clef: "treble", keys: ["d/5"], duration: "q", stem_direction: -1 })
                ],
